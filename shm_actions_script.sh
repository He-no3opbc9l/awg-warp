...
WARP_SETUP="/etc/amnezia/warp/warp_setup.sh"
...
apt install -y build-essential \
    ...
    libgmp-dev
        ...
        # Install and activate WARP tunnel before AWG init
        # (so awg-manager detects warp0.conf and enables double tunneling)
        if [ -f "$WARP_SETUP" ]; then
            echo "Setting up WARP tunnel..."
            chmod 700 $WARP_SETUP
            bash $WARP_SETUP install
        else
            echo "WARP setup script not found, skipping double tunneling"
        fi
